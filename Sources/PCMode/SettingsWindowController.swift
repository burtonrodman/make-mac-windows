...
/// The "Manage Excluded Apps" window for the Control+C/V remap feature —
/// shows and edits which apps are exempted from having their literal
/// Control+C/V remapped to Command+C/V (see `HotkeyEventTap` and
/// `Preferences.ctrlCVDenylistBundleIDs`). Terminal emulators and
/// remote-desktop/VM consoles are seeded in by default, since they need the
/// literal keystroke (SIGINT, raw paste-through, etc.) rather than Mac-style
/// copy/paste.
///
...
final class SettingsWindowController: NSWindowController {
    ...
    /// Brings the window to the front, refreshing its list from
    /// `Preferences` first in case it was edited elsewhere (e.g. a future
